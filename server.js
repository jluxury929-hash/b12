// ===============================================================================
// APEX TITAN v110.0 (ULTIMATE PINNACLE OVERLORD) - FINAL ENGINE
// ===============================================================================
// MERGE SYNC: v109.0 (UNIVERSAL) + v20.0 (PINNACLE MASTER) + AI PROFIT-GATE
// ===============================================================================
...
// --- SAFETY: GLOBAL ERROR HANDLERS (v20.0 RESILIENCE) ---
...
// --- CONFIGURATION (v20.0 ULTRA-SENSITIVE) ---
const GLOBAL_CONFIG = {
    ...
    // v20.0 STRATEGY SETTINGS
    MIN_WHALE_VALUE: 0.1,                // Trigger logs for any move > 0.1 ETH
...
async function fetchAIAssessment(ethPrice) {
    ...
    const userQuery = `ETH: $${ethPrice}. Suggest if Pinnacle strikes should be aggressive (BUY) or defensive (SELL).`;
    try {
...
// --- MASTER PROCESS (v20.0 Branding) ---
if (cluster.isPrimary) {
    console.clear();
    console.log(`${TXT.bold}${TXT.gold}
╔════════════════════════════════════════════════════════╗
║   ⚡ APEX TITAN v110.0 | ULTIMATE PINNACLE OVERLORD   ║
║   MODE: ULTRA-SENSITIVE WHALE HUNTER + AI ANALYST     ║
╚════════════════════════════════════════════════════════╝${TXT.reset}`);
    ...
    cluster.on('exit', (worker) => {
        console.log(`${TXT.red}⚠️  Engine ${worker.id} Died. Respawning...${TXT.reset}`);
...
async function initWorker(CHAIN) {
    ...
    async function safeConnect() {
        try {
            ...
            console.log(`${TXT.green}✅ ENGINE ${cluster.worker.id} SYNCED [${ROLE}] on ${TAG}${TXT.reset}`);
            ...
            process.on('message', (msg) => {
                ...
                if (msg.type === 'WHALE_SIGNAL' && msg.chainId === CHAIN.chainId && !isProcessing && ROLE !== "ANALYST") {
                    isProcessing = true;
                    strike(provider, wallet, apexIface, gasOracle, currentEthPrice, CHAIN, msg.target, msg.isPinnacle)
            ...
            if (DIVISION === 0 || DIVISION === 1) {
                wsProvider.on("pending", async (txHash) => {
                    ...
                    if (stochasticTrigger) {
                        process.send({ type: 'WHALE_SIGNAL', chainId: CHAIN.chainId, target: CHAIN.uniswapRouter, isPinnacle: false });
                    } else {
                        const tx = await provider.getTransaction(txHash).catch(() => null);
                        if (tx && tx.to) {
                            const val = tx.value ? parseFloat(formatEther(tx.value)) : 0;
                            const isPinnacle = val >= GLOBAL_CONFIG.WHALE_HUNTER_THRESHOLD;
                            const isStandard = val >= GLOBAL_CONFIG.MIN_WHALE_VALUE && tx.to.toLowerCase() === CHAIN.uniswapRouter.toLowerCase();
                            
                            if (isPinnacle || isStandard) {
                                process.send({ type: 'WHALE_SIGNAL', chainId: CHAIN.chainId, target: tx.to, isPinnacle });
                            }
                        }
                    }
                });
...
async function strike(provider, wallet, iface, gasOracle, ethPrice, CHAIN, target, isPinnacle) {
    try {
        ...
        const totalCostBuffer = l2GasCost + l1Fee + aaveFee + parseEther(GLOBAL_CONFIG.MIN_PROFIT_BUFFER);
        ...
        if (rawProfit > (totalCostBuffer * safetyMultiplier) / 100n) {
            const netProfit = rawProfit - (totalCostBuffer - parseEther(GLOBAL_CONFIG.MIN_PROFIT_BUFFER));
            
            if (isPinnacle) {
                console.log(`\n${TAG} ${TXT.red}${TXT.bold}🚨 PINNACLE STRIKE AUTHORIZED${TXT.reset}`);
            } else {
                console.log(`\n${TAG} ${TXT.green}💎 OPPORTUNITY AUTHORIZED${TXT.reset}`);
            }
            console.log(`   ↳ ⚖️ Scaling: ${formatEther(loanAmount)} ETH | Net: +${formatEther(netProfit)} ETH (~$${(parseFloat(formatEther(netProfit)) * ethPrice).toFixed(2)})${TXT.reset}`);
